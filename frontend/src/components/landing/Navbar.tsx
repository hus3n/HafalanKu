      ...
      <div className="max-w-7xl mx-auto rounded-2xl border border-border bg-card/95 backdrop-blur-md shadow-xl text-card-foreground px-6 py-3.5 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2.5 group">
          ...
          <span className="font-outfit font-extrabold text-xl tracking-tight text-foreground flex items-center gap-1">
            ...
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.8)]" />
          </span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm font-semibold">
          {navLinks.map((link) => (
            <motion.a
              ...
              href={link.href}
              whileHover={{ y: -2 }}
              className="text-foreground/90 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors relative py-1"
            >
              {link.label}
            </motion.a>
          ))}
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden mt-2 p-6 rounded-2xl border border-border bg-card shadow-2xl text-card-foreground space-y-4 max-w-7xl mx-auto"
          >
