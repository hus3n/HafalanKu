...
export class SantriExcelService {
  /**
   * Helper internal untuk scoping database berdasarkan role & organisasi
   */
  ...
  async previewBulkImport(
    ...
    const existingSantriNameSet = new Set(existingActiveSantri.map(s => s.name.trim().toLowerCase()));

    // Deteksi baris header
    let headerRowIdx = 1;
    for (let i = 1; i <= Math.min(10, worksheet.rowCount); i++) {
      const row = worksheet.getRow(i);
      const cell1 = String(row.getCell(1).value || '').toLowerCase();
      const cell2 = String(row.getCell(2).value || '').toLowerCase();
      if (cell1.includes('santri') || cell2.includes('wali') || cell1.includes('nama')) {
        headerRowIdx = i;
        break;
      }
    }

    // Identifikasi letak kolom secara fleksibel
    ...
    for (let c = 1; c <= worksheet.columnCount; c++) {
      const val = String(hRow.getCell(c).value || '').toLowerCase();
      if (val.includes('santri')) colSantri = c;
      else if (val.includes('wali') && (val.includes('hp') || val.includes('wa') || val.includes('telepon') || val.includes('phone') || val.includes('no'))) colHp = c;
      else if (val.includes('wali')) colWali = c;
      else if (val.includes('kelas') || val.includes('kelompok') || val.includes('group')) colKelas = c;
      else if (val.includes('hafalan') || val.includes('capaian') || val.includes('surat') || val.includes('juz')) colHafalan = c;
    }
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      const getVal = (col: number) => {
        ...
        if (typeof c.value === 'object' && 'text' in c.value) return String((c.value as any).text || '').trim();
        return String(c.value).trim();
      };

      const namaSantri = getVal(colSantri);
      const namaWali = getVal(colWali);
      let noHpWali = getVal(colHp);
      const namaKelas = getVal(colKelas);
      const capaianHafalan = getVal(colHafalan);
      ...
      // Validasi notasi hafalan
      let parsedHafalan: any[] = [];
      if (capaianHafalan) {
        try {
          parsedHafalan = parseHafalanNotation(capaianHafalan);
  ...
  async exportFullSantriData(
    ...
    sheet1.columns = [
      ...
      { key: 'totalSurat', width: 18 },
      { key: 'totalSetoran', width: 16 },
      { key: 'ringkasanCapaian', width: 42 },
    ];
    ...
    santris.forEach((s: any, idx: number) => {
      ...
      const memorizedSurahNumbers = Array.from(new Set((s.hafalan || []).map((h: any) => h.surahNumber as number)));
      const compressedNotation = compressHafalanNotation(s.hafalan || []);
      ...
      const row = sheet1.addRow([
        ...
        s.parentName,
        phone,
        memorizedSurahNumbers.length,
        s.hafalan?.length || 0,
        compressedNotation || 'Belum ada hafalan',
      ]);
...
export const santriExcelService = new SantriExcelService();
